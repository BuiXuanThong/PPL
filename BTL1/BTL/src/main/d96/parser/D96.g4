...
inside_decls: vardecls | funcdecls;
...
// vardecl: (VAR | VAL) listvar COLON typ (ASSIGN (exps_list | array_decl))? SEMI; // array decl ?????
...
stmt
	: if_stmt 
	| for_stmt
	| break_stmt | continue_stmt | ret_stmt
	| assign_stmt
	| vardecl_no_dollar
	| member_access_stmt
	| block_stmt
...
if_stmt: IF LP exp RP block_stmt (ELSEIF LP exp RP block_stmt)* (ELSE block_stmt)?;
