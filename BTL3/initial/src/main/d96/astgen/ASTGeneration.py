...
class ASTGeneration(D96Visitor):
    ...
    def visitMember_access_stmt(self, ctx: D96Parser.Member_access_stmtContext):
        ...
        if ctx.DOT():
            return CallStmt(self.visit(ctx.exp()) if ctx.exp() else SelfLiteral(), Id(ctx.ID(0).getText()) if ctx.ID(0) else Id(ctx.DOLLAR_ID().getText()), self.visit(ctx.exps_list()) if ctx.exps_list() else [])
        else:
            return CallStmt(Id(ctx.ID(0).getText()), Id(ctx.DOLLAR_ID().getText()) if ctx.DOLLAR_ID() else Id(ctx.ID(1).getText()), self.visit(ctx.exps_list()) if ctx.exps_list() else []) # con chua fix
    ...
    def visitCallExp1(self, ctx: D96Parser.CallExp1Context):
        ...
        if ctx.getChildCount() == 1:
            ...
            return FieldAccess(self.visit(ctx.callExp1()), Id(ctx.ID().getText()) if ctx.ID() else Id(ctx.DOLLAR_ID().getText()))
        else:
            return CallExpr(self.visit(ctx.callExp1()), Id(ctx.ID().getText()) if ctx.ID() else Id(ctx.DOLLAR_ID().getText()), self.visit(ctx.exps_list()) if ctx.exps_list() else [])
    ...
    def visitSelfExp(self, ctx: D96Parser.SelfExpContext):
        ...
        if ctx.getChildCount() == 1:
            ...
            return CallExpr(SelfLiteral(), Id(ctx.ID().getText()), self.visit(ctx.exps_list()) if ctx.exps_list() else [])
        else:
    ...
    def visitCallExp2(self, ctx: D96Parser.CallExp2Context):
        ...
        if ctx.getChildCount() == 1:
            ...
            return FieldAccess(Id(ctx.ID(0).getText()), Id(ctx.DOLLAR_ID().getText()) if ctx.DOLLAR_ID() else Id(ctx.ID(1).getText()))
        else:
            return CallExpr(Id(ctx.ID(0).getText()), Id(ctx.DOLLAR_ID().getText()) if ctx.DOLLAR_ID() else Id(ctx.ID(1).getText()), self.visit(ctx.exps_list()) if ctx.exps_list() else [])
